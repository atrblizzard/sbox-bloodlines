...
public class DevMenu : Panel
{
	public DevMenu()
	{
		...
		panel.Add.Button( "Change Team", "button", () => ConsoleSystem.Run( "v_open_menu_team" ) );
		panel.Add.Button( "Change Class", "button", () => ConsoleSystem.Run( "v_open_menu_class" ) );
		panel.Add.Button( "Suicide", "button", () => ConsoleSystem.Run( "kill" ) );
		panel.Add.Button( "Regenerate", "button", () => ConsoleSystem.Run( "v_regenerate" ) );
		...
		panel.Add.Button( "Spawn a Dummy Bot", "button", () => ConsoleSystem.Run( "v_bot_add" ) );
	}

	public override void Tick()
	{
		if ( Input.Released( "Slot0" ) )
			Mouse.Position = Screen.Size * .5f;
		SetClass( "open", Input.Down( "Slot0" ) );
	}
}
