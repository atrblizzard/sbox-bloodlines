...
namespace SWB_Base
{
    ...
    {
        public virtual string ViewModelHandPath => null;
        public BaseViewModel ViewModelHandEntity { get; protected set; }

        public virtual int WeaponEnum => 1;
        ...
        public override void ActiveStart(Entity ent)
        {
            ...
            if (IsLocalPawn)
            {
                ...
                if (Primary.Ammo == 0 && !string.IsNullOrEmpty(Primary.DrawEmptyAnim))
                {
                    ...
                    ViewModelHandEntity?.SetAnimBool(Primary.DrawEmptyAnim, true);
                }
                else if (!string.IsNullOrEmpty(Primary.DrawAnim))
                {
					if ( activeViewModel.HasAnimGraph() )
						activeViewModel?.SetAnimBool( Primary.DrawAnim, true );
					else
						activeViewModel.CurrentSequence.Name = DrawSequence;

					ViewModelHandEntity?.SetAnimBool(Primary.DrawAnim, true);
        ...
        public virtual void StartReloadEffects(bool isEmpty)
        {
			...
			else if (Primary.ReloadAnim != null)
            {
				...
				if (reloadingViewModel.HasAnimGraph())
					reloadingViewModel?.SetAnimBool(Primary.ReloadAnim, true);
				else
					reloadingViewModel.CurrentSequence.Name = ReloadSequence;
				
				ViewModelHandEntity?.SetAnimBool(Primary.ReloadAnim, true);                
			}
        ...
        public override void CreateViewModel()
        {
            ...
            ViewModelEntity.Name = "ViewModelEntity";
            ...
            if (string.IsNullOrEmpty(ViewModelHandPath))
            {
                Log.Error($"{ViewModelHandPath} cannot be found!");
                return;
            }

            // Shit
            ViewModelHandEntity = new ViewModelBase(this)
            {
            ...
            ViewModelHandEntity.SetModel(ViewModelHandPath);
            ViewModelHandEntity.SetParent(ViewModelEntity, true);
            ViewModelHandEntity.SetAnimInt("weapon", WeaponEnum);
        }
        ...
        public virtual void SendWeaponAnim(string anim, bool value = true)
        {
			...
			if ( ViewModelEntity != null )
				ViewModelEntity.CurrentSequence.Name = anim;			
		}
        ...
        public override void DestroyViewModel()
        {
            ...
            ViewModelHandEntity?.Delete();
            ViewModelHandEntity = null;
        }
    }
}
